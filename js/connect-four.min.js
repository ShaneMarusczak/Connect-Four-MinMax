...
document.getElementById("fc"+a.cellIndex).classList.remove("bounce"),window.sleep(800).then(function(){1===c.round&&c.generateComputerDecision()}))};k.animateDrop=function(a){a=void 0===a?{}:a;var c=a.inputRow,b=a.inputCol,d=a.moveTurn,f=void 0===a.currentRow?0:a.currentRow;f===c?(h||d||(window.sleep(75).then(function(){window.modalOpen("Thinking...")}),document.getElementsByTagName("html")[0].classList.add("progressCursor")),d&&(document.getElementById("uiBlocker").classList.remove("block"),n=!1),
document.getElementById("td"+f+b).className=d?"coin cpu-coin":"coin human-coin"):(n=!0,document.getElementById("td"+f+b).classList.add("coin"),document.getElementById("td"+f+b).classList.add(d?"cpu-coin":"human-coin"),window.sleep(100).then(function(){document.getElementById("td"+f+b).classList.remove("coin");document.getElementById("td"+f+b).classList.remove(d?"cpu-coin":"human-coin")}),window.sleep(100).then(function(){k.animateDrop({currentRow:f+1,inputCol:b,inputRow:c,moveTurn:d})}))};k.prototype.playCoin=
function(a){if(!h){for(var c=this.rows-1;0<=c;c--)if(document.getElementById("gameBoard").rows[c].cells[a].classList.contains("empty")){1===this.round?k.animateDrop({inputCol:a,inputRow:c,moveTurn:!0}):k.animateDrop({inputCol:a,inputRow:c,moveTurn:!1});break}this.board.canPlace(a)?(this.round=0===this.round?1:0,this.checkGameOver()):(document.getElementById("uiBlocker").classList.remove("block"),window.modal("Invalid move!",2E3))}};k.prototype.quickMove=function(){for(var a=0;a<this.columns;a++){var c=
this.board.getBoardCopy();if(!c.columnIsFull(a)&&(c.placeInColumnForQuickMove(a,1),1E5===c.evaluateScore()))return a}for(a=0;a<this.columns;a++)if(c=this.board.getBoardCopy(),!c.columnIsFull(a)&&(c.placeInColumnForQuickMove(a,0),-1E5===c.evaluateScore()))return a;return-1};k.prototype.generateComputerDecision=function(){var a=this;if(!h){var c=0,b=this.quickMove(),c=-1!==b?b:$jscomp.makeIterator(this.maximize(this.board,this.getDepth())).next().value;window.sleep(325*(14/Number(this.getDepth()))).then(function(){window.modalClose();
window.sleep(100).then(function(){return a.playCoin(c)})})}};k.prototype.getDepth=function(){return 4>=Number(this.depth)?this.depth:5>this.turnsTaken?this.depth-2:this.depth};k.prototype.maximize=function(a,c,b,l){var f=a.evaluateScore();if(a.isFinished(c,f))return[null,f];for(var f=[null,-99999],e=$jscomp.makeIterator(d),g=e.next();!g.done;g=e.next()){var g=g.value,h=a.getBoardCopy();if(h.canPlace(g)){h=this.minimize(h,c-1,b,l);if(null===f[0]||h[1]>f[1])f[0]=g,b=$jscomp.makeIterator(h),b.next(),
